...
export function renderEvent() {
  ...
  events.forEach(e=>{
    ...
    card.innerHTML=`${e.banner ? `<img src="${esc(e.banner)}" alt="${esc(e.name)}" style="width:100%;border-radius:16px">` : ''}
      ...
      <div class="event-stages">${stages.map(s=>`<button class="btn block mt8" data-stage="${s.id}">${esc(s.name)}<br>⚡${s.stamina} ・ ${s.floors.length}フロア ・ 🍬${s.currencyDrop?.amount || 0}個確定${state.progress[s.id]?.normal ? ' ✓' : ''}</button>`).join('')}</div>
